 ...
 * Copyright 2020-2022 Brown Book Software, LLP
 */
...
class TermInput extends React.Component {
  ...
  showCalendarAvailable() {
    if (this.state.showCalendarAvailable === false) {
      this.setState({ showCalendarCensus: false, showCalendarStart: false, showCalendarEnd: false });
    }
  ...
  showCalendarCensus() {
    if (this.state.showCalendarCensus === false) {
      this.setState({ showCalendarAvailable: false, showCalendarStart: false, showCalendarEnd: false });
    }
  ...
  showCalendarStart() {
    if (this.state.showCalendarStart === false) {
      this.setState({ showCalendarCensus: false, showCalendarAvailable: false, showCalendarEnd: false });
    }
  ...
  showCalendarEnd() {
    if (this.state.showCalendarEnd === false) {
      this.setState({ showCalendarCensus: false, showCalendarAvailable: false, showCalendarStart: false });
    }
    this.setState({ showCalendarEnd: !this.state.showCalendarEnd });
  }

  handleSubmit() {
    const tcode = this.tcodeRef.current.value.trim();
    const stype = this.stypeRef.current.value.trim();
    const descr = this.descRef.current.value.trim();
    const census = this.censusRef.current.value.trim();
    const available = this.availableRef.current.value.trim();
    const start = this.startRef.current.value.trim();
    const end = this.endRef.current.value.trim();
    const ugradOver = this.ugradOverRef.current.value.trim();
    const gradOver = this.gradOverRef.current.value.trim();
  ...
  render() {
    let censusCalendar = null;
    let availableCalendar = null;
    let startCalendar = null;
    let endCalendar = null;

    if (this.state.showCalendarAvailable) {
      availableCalendar = <Calendar onChange={this.onChangeAvailable} value={this.state.availableDateInput} calendarType="US" />;
    }
    if (this.state.showCalendarCensus) {
      censusCalendar = <Calendar onChange={this.onChangeCensus} value={this.state.censusDateInput} calendarType="US" />;
    }
    if (this.state.showCalendarStart) {
      startCalendar = <Calendar onChange={this.onChangeStart} value={this.state.startDateInput} calendarType="US" />;
    }
    if (this.state.showCalendarEnd) {
      endCalendar = <Calendar onChange={this.onChangeEnd} value={this.state.endDateInput} calendarType="US" />;
    }
    ...
    return (
      <div>
        ...
        <div className="row mb-2">
          <div className="col-sm-6">
            <div ref={this.availableCalendarRef}>
          ...
          <div className="col-sm-6">
            <div ref={this.censusCalendarRef}>
...
TermInput.propTypes = {
  onTermCreate: PropTypes.func.isRequired
};
