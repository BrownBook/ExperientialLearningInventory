...
function generateContractHandler() {
  ...
  $.post(
    'index.php?module=intern&action=SaveInternship',
    $('#internship').serialize() + '&generateContract=true',
    function (data) {
...
function initFacultySelector() {
  ...
  $('#internship_department').bind('change', function () {
    ...
    $.ajax({
      success: function (data) {
        if (data.length === 0) {
          $.ajax({
            ...
            data: { module: 'intern', action: 'restFacultyById', id: $('#internship_faculty_id').val() },
            dataType: 'json',
            url: 'index.php'
          });
        } else {
      ...
      data: { module: 'intern', action: 'getFacultyListForDept', department: $('#internship_department').val() },
      dataType: 'json',
      url: 'index.php'
    });
  });
  ...
  function selectFaculty(bannerId) {
    ...
    for (var i = 0; i < facultyData.length; i++) {
      if (facultyData[i].id === bannerId) {
        // NB: Comparison must use double equals, not triple
    ...
    const departmentName = $('#internship_department :selected').text();
